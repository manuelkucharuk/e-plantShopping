...
const CartItem = ({ onContinueShopping , setAddedToCart}) => {
  ...
  const calculateTotalAmount = () => cart.reduce((total,prod)=> total + prod.cost*prod.quantity,0)
  ...
  const calculateTotalCost = (item) => item.cost*item.quantity
    const countAddedToCart = () => cart.reduce((count,item)=>count+item.quantity,0)


  return (
      <div className="cart-container">
          <h2 style={{color: 'black'}}>Total Cart Amount: ${calculateTotalAmount()}</h2>
          <h2 style={{color: 'black'}}>Amount of items: {countAddedToCart()}</h2>
          <div>
              {cart.map(item => (
